...
export default function RestaurantMenu() {
  ...
  const fetchMenuItem = async () => {
    try {
      const res = await api.get(import.meta.env.VITE_RESTAURANT_MENU_ITEMS);
    ...
    <div className="p-4 md:p-8 bg-[#FAF7F2] min-h-screen font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="md:col-span-2 relative">
          <Search
            className="absolute left-4 top-1/2 -translate-y-1/2 text-[#A3485A]"
            size={20}
          />
          <input
            type="text"
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-[#842A3B] text-[#F5DAA7]">
              <th className="p-6 font-bold uppercase text-[20px] tracking-[0.2em] ps-10">
                Ref No.
              </th>
              <th className="p-6 font-bold uppercase text-[20px] tracking-[0.2em]">
                Dish Details
              </th>
              <th className="p-6 font-bold uppercase text-[20px] tracking-[0.2em]">
                Category
              ...
              <th className="p-6 font-bold uppercase text-[20px] tracking-[0.2em] text-center">
                Actions
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {filteredItems.length > 0 ? (
              filteredItems.map((item, index) => (
                <tr
                  key={item._id || index}
                  className="hover:bg-[#F5DAA7]/5 transition-colors group"
                >
                  <td className="p-6 ps-10  font-mono text-black">
                    {index + 1}
                  </td>
                  <td className="p-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] font-bold text-[#A3485A] uppercase tracking-widest mt-0.5">
                          {item.type}
                        </div>
                      </div>
                    </div>
              ...
              <tr>
                <td colSpan="5" className="p-20 text-center">
                  <p className="text-gray-300 italic">
                    No dishes have been added to the collection yet.
                  </p>
                </td>
              </tr>
            )}
      ...
      {/* Modals Mapping */}
      {openItemModal && (
        <AddMenuModal onClose={() => setOpenItemModal(false)} />
      )}
      {viewItemModal && (
        <ViewItemModal
